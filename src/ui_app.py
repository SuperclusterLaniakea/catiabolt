...
功能：
  - 左侧类型树：按大类分组（六角/方形头 · 内六角 · 圆/沉头 · 特殊）选择螺栓类型
  - 右侧参数表单：根据类型动态生成，含单位、范围校验、国标尺寸预设自动填充；
    支持螺纹公差带(4g~10g)、产品等级(A/B/C)、性能等级(4.6~12.9)选择
  - 螺纹标准区：公制(粗牙/细牙/超细牙) / 英制(UNC/UNF/UNRF/BSW) 任意切换，
    自动带出 D、P、牙型角并估算头部尺寸
  - 实时 2D 顶视预览：头部外轮廓 + 杆部圆，随参数即时更新
...
from bolt_data import (
    BOLT_TYPES, BOLT_CATEGORIES, PARAM_META, default_params, apply_preset, derive_e,
    THREAD_SYSTEMS, list_thread_systems, thread_sizes, get_thread_spec, estimate_hex_dims,
)
from bolt_geometry import build_model, list_all_types


class BoltDesigner(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("CATIA 螺栓参数化建模工具（全种类 · 全精度 · 公英制）")
    ...
    def _init_ui(self):
        ...
        # 左：类型树 + 2D 顶视预览（左下角）
        left = QVBoxLayout()
        left.addWidget(QLabel("<b>螺栓类型</b>"))
        ...
        hint = QLabel("<span style='color:#888;font-size:10px'>选中标准规格将自动带出螺距/头部尺寸/长度；"
                      "也可在下方修改任意参数（长度 L、螺纹长度 L₁、倒角等）后点击“绘制到 CATIA”。</span>")
        hint.setWordWrap(True)
        mid.addWidget(hint)

        # 螺纹标准 / 规格（公制粗牙·细牙·超细牙 / 英制 UNC·UNF·UNRF·BSW）
        thread_box = QGroupBox("螺纹标准（公制 / 英制 · 粗牙 / 细牙 / 超细牙）")
        ...
        # 精细程度提示（公差带/产品等级/性能等级在下方参数表单中选择）
        prec_hint = QLabel("<span style='color:#888;font-size:10px'>精细程度：螺纹公差带 "
                           "(4g~10g) / 产品等级 (A·B·C) / 性能等级 (4.6~12.9) "
                           "在下方“参数”表单中设置。</span>")
        prec_hint.setWordWrap(True)
        thread_layout.addWidget(prec_hint)
    ...
    def _build_param_form(self, key):
        ...
        if "s" in order and "e" in PARAM_META and "e" not in order:
            order.append("e")
        # 全局附加参数：螺纹牙型角 / 倒圆 / 精度选择（所有类型通用，始终显示）
        for gk in ("thread_angle", "fillet", "fillet_r", "thread_class",
                   "product_grade", "performance"):
        ...
        for pkey in order:
            ...
            if meta.get("type") == "bool":
                ...
                w.stateChanged.connect(lambda *_: self._on_param_changed())
                self.form_layout.addRow(label, w)
            elif meta.get("type") == "choice":
                w = QComboBox()
                for ck in meta.get("choices", []):
                    w.addItem(ck)
                cur = params.get(pkey, meta.get("default", ""))
                if w.findText(str(cur)) >= 0:
        ...
        try:
            for pkey, (w, meta) in self.param_widgets.items():
                ...
                if meta.get("type") == "bool":
                    ...
                    if w.findText(str(params[pkey])) >= 0:
                        w.setCurrentText(str(params[pkey]))
                else:
    ...
    def _refresh_derived(self):
        """刷新派生量：e（按形状）。不改变规格下拉框。"""
        shape = BOLT_TYPES[self.current_key].get("shape", "hex_head") if self.current_key else "hex_head"
    ...
    def _on_thread_size_changed(self, size_label):
        ...
        self._set_field("D", d)
        self._set_field("P", p)
        # 公制且预设命中 → 带出头部尺寸；否则按比例估算
        hit = size_label in BOLT_TYPES[self.current_key].get("presets", {})
        if hit:
            params = apply_preset(self.current_key, size_label)
            for pk in ("s", "k", "dk", "dm1", "L", "L1"):
                if pk in params:
                    self._set_field(pk, params[pk])
            self._set_field("D", d)
            self._set_field("P", p)
        else:
            s_est, k_est = estimate_hex_dims(d)
            self._set_field("s", s_est)
            self._set_field("k", k_est)
            if "L" in self.param_widgets:
                self._set_field("L", max(20.0, d * 5))
            if "L1" in self.param_widgets:
                cur_L = self.param_widgets["L"][0].value() if "L" in self.param_widgets else 40.0
                cur_k = self.param_widgets["k"][0].value() if "k" in self.param_widgets else k_est
                self._set_field("L1", max(1.0, cur_L - cur_k))
        self.preset_combo.blockSignals(True)
        self.preset_combo.setCurrentText("自定义")
        self.preset_combo.blockSignals(False)
        self._refresh_derived()
    ...
    def _render_preview(self, pv, size=180):
        ...
        max_r = max(max(abs(v) for v in xs), max(abs(v) for v in ys), shank_r, 1e-6)
        margin = 16
